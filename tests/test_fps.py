...
class TestFPS(unittest.TestCase):
    ...
    def test_fps_basic(self):
        ...
        self.assertEqual(A/B,fps.FPS([748683265, 811073537, 857866241, 892960768, 928055297, 963149825]))
        ...
        A=fps.FPS([1,2,3])
        A/=B
        self.assertEqual(A,fps.FPS([748683265, 811073537, 857866241, 892960768, 928055297, 963149825]))
        A=fps.FPS([1,2,3])
    ...
    def test_fps_exp(self):
        A=fps.FPS([0,1,2,3,4])
        self.assertEqual(A.exp().resize(5),fps.FPS([1,1,499122179,166374064,291154613]))
    ...
    def test_sqrt(self):
        N=10**4
        mod=998244353
        seq=[pow(random.randrange(1,mod),2,mod)]+[random.randrange(mod) for i in range(N-1)]
        A=fps.FPS(seq)
        B=A.sqrt()
        C=B*B
        D=C.resize(N)
    ...
    def test_fps_inverse(self):
        A=fps.FPS([5,4,3,2,1])
        self.assertEqual(A.inv(),fps.FPS([598946612,718735934,862483121,635682004,163871793]))
